...
function wizors_investments_resize_actions(cont) {
	...
	wizors_investments_resize_video( cont );
...
function wizors_investments_resize_video(cont) {
	"use strict";
	if (cont === undefined) {
		cont = jQuery( 'body' );
	}
	cont.find( 'video' ).each(
		function() {
			...
			var video     = jQuery( this ).addClass( 'wizors_investments_resize' ).eq( 0 );
			...
			if ( percent ) {
				...
				if ( mejs_cont.length > 0 ) {
					wizors_investments_set_mejs_player_dimensions( video, w_real, h_real );
				}
			}
	...
	cont.find( '.video_frame iframe,iframe' ).each(
		function() {
			// If item now invisible
			if (jQuery( this ).hasClass( 'trx_addons_resize' ) || jQuery( this ).hasClass( 'trx_addons_noresize' ) || jQuery( this ).addClass( 'wizors_investments_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0) {
				return;
			}
...
function wizors_investments_set_mejs_player_dimensions(video, w, h) {
	"use strict";
	if (mejs) {
        ...
        if (window.mejs) {
            ...
            cont.find('audio:not(.inited),video:not(.inited)').each(function() {
				...
				if (jQuery(this).parents('.mejs-mediaelement').length == 0
					...
					&& (WIZORS_INVESTMENTS_STORAGE['init_all_mediaelements'] || (!jQuery(this).hasClass('wp-audio-shortcode') && !jQuery(this).hasClass('wp-video-shortcode') && !jQuery(this).parent().hasClass('wp-playlist')))) {
