...
?>">
	...
	if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
		set_query_var('trx_addons_args_featured', array(
			...
			'hover' => 'zoomin',
			'thumb_size' => apply_filters('trx_addons_filter_services_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'serv' : 'serv'))
		));
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button "><?php esc_html_e('More info', 'wizors-investments'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
