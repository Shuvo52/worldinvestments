...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="<?php esc_attr__('img', 'wizors-investments'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
    ...
    if (!empty($args['price'])) {
        ...
        ?><div class="sc_price_description"><?php
        if (!empty($args['currency'])) {
            ?><span class="sc_price_currency"><?php echo esc_html($args['currency']); ?></span><?php
        }
        ?><span class="sc_price_value"><?php echo wp_kses($parts[0], 'wizors_investments_kses_content'); ?></span><?php
        if (count($parts) > 1 && $parts[1]!='') {
            ?><span class="sc_price_decimals"><?php echo wp_kses($parts[1], 'wizors_investments_kses_content'); ?></span><?php
        }
        ?></div><?php
    }
		if (!empty($args['description'])) {
			?><div class="sc_price_price"><?php echo wp_kses(trx_addons_parse_codes($args['description']), 'wizors_investments_kses_content'); ?></div><?php
		}

		if ( ($content = do_shortcode($content))!='' ) {
			?><div class="sc_price_details"><?php echo wp_kses($content, 'wizors_investments_kses_content'); ?></div><?php
		}
